...
exports.handler = async (event, context) => {
  ...
  if (!userRecord.emailVerified) {
    return {
      ...
      const bleed = body.bleed; // Bleed option (true/false)
      ...
      // Generate the JPEG image using sharp with the requested resolution and bleed
      let image = sharp(imageBuffer).resize(widthPixels, heightPixels);
      if (bleed) {
        const bleedInPixels = Math.round(dpi * bleed); // Round to the nearest integer
        ...
        image = image.extend({
          ...
          left: bleedInPixels,
          right: bleedInPixels,
          background: { r: 255, g: 255, b: 255, alpha: 1 }, // White bleed area
        });
      }
      ...
      const fileName = `imagesExport/${Date.now()}.jpeg`;
